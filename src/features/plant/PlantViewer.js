  ...
  loadGLBFromUrl(url, onLoadCallback, onErrorCallback) {
    ...
    loader.load(
      ...
      (gltf) => {
        ...
        if (this.loadedCustomModel) {
          this.plantGroup.remove(this.loadedCustomModel);
        }
        ...
        model.traverse((obj) => {
          if (obj.isMesh) {
            ...
            // Tag the custom model mesh so click-to-learn works on it
        ...
        this.loadedCustomModel = model;

        console.log('[PlantViewer] Loaded GLB successfully:', url);
        if (onLoadCallback) onLoadCallback();
      },
      undefined,
      (error) => {
        console.error('[PlantViewer] Error loading GLB:', error);
